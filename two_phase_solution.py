...
plt.scatter(x_ac_l, t_b, marker='x', label='X_Ac_l')
plt.scatter(x_ac_v, t_b, marker='o', label='X_Ac_v')
plt.xlim([1, 0])
plt.xlabel('X_ac')
plt.ylabel('T_b')
plt.legend(loc= 2)
...
# # fitting a 3rd order curve to the data
...
# # Fitting a 2nd order curve to the data
...
p, co = curve_fit(f, x_ac_l1, t_b1)
p2, co2 = curve_fit(f, x_ac_v1, t_b1)
p3, co3 = curve_fit(f, x_ac_l2, t_b2)
p4, co4 = curve_fit(f, x_ac_v2, t_b2)


plt.scatter(x_ac_l, t_b, marker='x')
plt.scatter(x_ac_v, t_b, marker='o')

fit_l1 = lambda x: f(x, p[0], p[1], p[2])
fit_v1 = lambda x: f(x, p2[0], p2[1], p2[2])
fit_l2 = lambda x: f(x, p3[0], p3[1], p3[2])
fit_v2 = lambda x: f(x, p4[0], p4[1], p4[2])

x1 = np.linspace(x_ac_l1[0], 0.4471, 100)
x2 = np.linspace(x_ac_v1[0], 0.4471, 100)
x3 = np.linspace(x_ac_l2[0], 0.4471, 100)
x4 = np.linspace(x_ac_v2[0], 0.4471, 100)



plt.plot(x1, fit_l1(x1), c='b', label='X_Ac(l)')
plt.plot(x2, fit_v1(x2), c='r', label='X_Ac(v)')
plt.plot(x3, fit_l2(x3), c='b')
plt.plot(x4, fit_v2(x4), c='r')
